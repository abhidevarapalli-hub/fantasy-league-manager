...
export const useRealtimeGame = () => {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            setPlayers(prev => prev.map(p => p.id === payload.new.id ? mapDbPlayer(payload.new as Tables<'players'>) : p));
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            setManagers(prev => prev.map(m => m.id === payload.new.id ? mapDbManager(payload.new as Tables<'managers'>) : m));
          } else if (payload.eventType === 'DELETE') {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            setSchedule(prev => prev.map(s => s.id === payload.new.id ? mapDbSchedule(payload.new as Tables<'schedule'>) : s));
          } else if (payload.eventType === 'DELETE') {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT') {
            ...
            setActivities(prev => prev.map(a => a.id === payload.new.id ? mapDbTransaction(payload.new as Tables<'transactions'>) : a));
          } else if (payload.eventType === 'DELETE') {
  ...
  const getManagerRosterCount = useCallback((managerId: string) => {
  ...
  const addNewPlayer = async (name: string, team: string, role: 'Batsman' | 'Bowler' | 'All Rounder' | 'Wicket Keeper') => {
