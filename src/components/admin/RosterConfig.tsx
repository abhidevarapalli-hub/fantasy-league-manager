...
const CONFIG_FIELDS: ConfigField[] = [
  ...
  { key: 'minBatWk', label: 'Min BAT / WK', description: 'Minimum Batsmen/Wicket Keepers required', min: 0, max: 11 },
  { key: 'maxBatWk', label: 'Max BAT / WK', description: 'Maximum Batsmen/Wicket Keepers allowed', min: 1, max: 11 },
  { key: 'minBowlers', label: 'Min Bowlers', description: 'Minimum bowlers required', min: 0, max: 11 },
  { key: 'maxBowlers', label: 'Max Bowlers', description: 'Maximum bowlers allowed', min: 1, max: 11 },
  { key: 'minAllRounders', label: 'Min All-Rounders', description: 'Minimum all-rounders required', min: 0, max: 11 },
  { key: 'maxAllRounders', label: 'Max All-Rounders', description: 'Maximum all-rounders allowed', min: 1, max: 11 },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {CONFIG_FIELDS.map((field) => (
            <div key={field.key} className="space-y-1">
              ...
              <Input
                ...
                min={field.min}
                max={field.max}
                value={editedConfig[field.key] as number}
          ...
          <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg border border-border">
            <div className="space-y-0.5">
              <Label className="text-sm font-medium text-foreground">Require Wicket Keeper</Label>
              <p className="text-xs text-muted-foreground">Force at least 1 WK in roster</p>
            </div>
            <Switch
              checked={editedConfig.requireWk}
              onCheckedChange={(checked) => setEditedConfig(prev => ({ ...prev, requireWk: checked }))}
            />
          </div>
        </div>
        ...
        <div className="bg-muted/50 rounded-lg p-3 border border-border space-y-1">
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span className={`font-mono font-bold ${!validationResult.isValid ? 'text-red-500' : 'text-green-500'}`}>
              {editedConfig.minBatWk + editedConfig.minBowlers + editedConfig.minAllRounders} / {editedConfig.activeSize}
            </span>
          </div>
        </div>
